...
class Trainer:
    def __init__(self, options):
        """
        Initializes Trainer object with specified options.
        ...
        # Check if height and width are multiples of 32
        if self.opt.height % 32 != 0:
            logger.error("'height' must be a multiple of 32")
            raise ValueError("'height' must be a multiple of 32")
        if self.opt.width % 32 != 0:
            logger.error("'width' must be a multiple of 32")
        ...
        if self.opt.frame_ids[0] != 0:
            logger.warning("frame_ids should start with 0")
            self.opt.frame_ids.insert(0, 0)
        ...
        if self.opt.use_depth_hints:
            if 's' not in self.opt.frame_ids:
                logger.error("Can't use depth hints without training from stereo images - either add --use_stereo or remove --use_depth_hints.")
                raise ValueError("Can't use depth hints without training from stereo images - either add --use_stereo or remove --use_depth_hints.")

        # Initialize encoder and depth decoder networks
        ...
        # Initialize pose encoder and decoder networks if using pose net
        ...
        # Initialize predictive mask network if using predictive mask
        if self.opt.predictive_mask:
            if not self.opt.disable_automasking:
                logger.warning("When using predictive_mask, please disable automasking with --disable_automasking")
        ...
        logger.info("Models and tensorboard events files are saved to: %s", self.opt.log_dir)
        logger.info("Training is using: %s", self.device)
    ...
    def predict_poses(self, inputs, features):
        """
        Predicts poses between input frames for monocular sequences.
        ...
        if self.num_pose_frames == 2:
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    # Predict axis angle and translation
                    axisangle, translation = self.models["pose"](pose_inputs)

                    # Store predicted axis angle, translation, and camera transformation
    ...
    @staticmethod
    def compute_sparse_loss(pred, target, valid_pixels, loss_mask):

        depth_hint_loss = torch.log(torch.abs(target - pred) + 1) * valid_pixels

        # only keep pixels where depth reprojection loss is smallest
    ...
    def compute_loss_masks(reprojection_loss, identity_reprojection_loss,
        ...
        if identity_reprojection_loss is None:
            ...
            depth_hint_loss_mask = (idxs == 2).float() if depth_hint_reprojection_loss is not None else None
    ...
    def compute_losses(self, inputs, outputs):
        """
        Computes the reprojection, smoothness and sparse losses for a minibatch.
        """
        ...
        if self.opt.use_depth_hints:
            ...
            depth_hint_reproj_loss = self.compute_reprojection_loss(pred,
                    inputs[('color', 0, 0)])
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                ...
                reprojection_losses.append(self.compute_reprojection_loss(pred,
                        target))
            ...
            if not self.opt.disable_automasking:
                ...
                for frame_id in self.opt.frame_ids[1:]:
                    ...
                    identity_reprojection_losses.append(self.compute_reprojection_loss(pred,
                            target))
                ...
                if self.opt.avg_reprojection:
                    identity_reprojection_loss = \
                        identity_reprojection_losses.mean(1, keepdim=True)
                else:

                    # Differently to Monodepth2, compute mins as we go

                    (identity_reprojection_loss, _) = \
                        torch.min(identity_reprojection_losses, dim=1,
                                keepdim=True)
            else:
                ...
                if self.opt.predictive_mask:
                    ...
                    if not self.opt.v1_multiscale:
                        mask = F.interpolate(mask, [self.opt.height,
                                self.opt.width], mode='bilinear',
                                align_corners=False)
                    ...
                    weighting_loss = 0.2 * nn.BCELoss()(mask,
                            torch.ones(mask.shape).cuda())
                    loss += weighting_loss.mean()

            if self.opt.avg_reprojection:
                reprojection_loss = reprojection_losses.mean(1,
                        keepdim=True)
            else:

                # Differently to Monodepth2, compute mins as we go

                (reprojection_loss, _) = torch.min(reprojection_losses,
                        dim=1, keepdim=True)
            ...
            if not self.opt.disable_automasking:
                ...
                identity_reprojection_loss += \
                    torch.randn(identity_reprojection_loss.shape).cuda() \
                    * 0.00001
            ...
            outputs['identity_selection/{}'.format(scale)] = (1
                    - reprojection_loss_mask).float()
            ...
            if self.opt.use_depth_hints:
                ...
                depth_hint_loss = self.compute_sparse_loss(pred, target,
                ...
                outputs['depth_hint_pixels/{}'.format(scale)] = \
